...
w2v_model = Word2Vec.load(r'/root/cyber_word2vec/1million.word2vec.model')
...
class WordSwapWordHowNetSyn(WordSwap):
    ...
    def __init__(self, language="eng"):
        ...
        self.entity_check_model = cyner.CyNER(transformer_model=r'/root/ckpt_20', use_heuristic=False, flair_model=None)        ###we use the "xlm-roberta" model to check whether entities are introduced
    ...
    def adjustment_replace_words(self,substitution_word,original_word,word_part_of_speech,current_text, index_now):
        ...
        substitution_word = wnl.lemmatize(substitution_word,self.standard_map[self._enptb_to_universal[word_part_of_speech]])
        ...
        if self._enptb_to_universal[word_part_of_speech] == "NOUN":
            ...
            try:
                aeiou_n = True if current_text.words[index_now - 1] in ["an", "An"] else False  # starts with a vowel
            except IndexError:                                                                                                # Adjectives begin
                aeiou_n = False
            if aeiou_n:
                if substitution_word[0] in "aeiou":  # Satisfied
    ...
    def _get_replacement_words(self, word, word_part_of_speech, current_text, index_now):
        ...
        for i in OpenHowNet_synonym_candidates:
            ...
            try:
                if "_" not in i \
                        and "-" not in i \
                        and almost_OpenHowNet_synonym_substitute != None \
                        and almost_OpenHowNet_synonym_substitute != word\
                        and w2v_model.wv.similarity(wnl.lemmatize(word,self.standard_map[self._enptb_to_universal[word_part_of_speech]]), wnl.lemmatize(i, self.standard_map[self._enptb_to_universal[word_part_of_speech]])) > threshold:
                    print(almost_OpenHowNet_synonym_substitute)
            ...
            for syn_word in syn.lemma_names():
                ...
                try:
                    if (
                        ...
                        and w2v_model.wv.similarity(wnl.lemmatize(word,self.standard_map[self._enptb_to_universal[word_part_of_speech]]), wnl.lemmatize(syn_word, self.standard_map[self._enptb_to_universal[word_part_of_speech]])) >threshold
                    ):
                        # WordNet can suggest phrases that are joined by '_' but we ignore phrases.
                        print(almost_substitute)
