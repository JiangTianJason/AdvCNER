...
def get_tokens(text, tokenizer):
    ...
    ######Suitable for models: "xlm-roberta" and "albert"########
    tokenized = tokenizer.tokenize(text)
    tokenized = ["<s>"] + tokenized + ["</s>"]
    return tokenized

    ######Original code. Suitable for the model: "bert"########
...
def postprocess_ner_output(original_text, predictions, out_tokens):
    ...
    #### Remove ## (BERT) BPE artifacts，Original code. Suitable for the model: "bert"#####
    ...
    # print("single_char_text:",single_char_text)
    prediction_idx = 0
    current_out_token = ""
    ...
    for token in original_tokens:
        ...
        ####Suitable for models: "bert" and "albert"，(lower cases in the tokenizer)####
        # token = token.lower()
        ########################################

        token_start_index = prediction_idx

        # print("current_out_token:",current_out_token)
        # print("token:",token)
        while current_out_token != token:
            current_out_token = f"{current_out_token}{single_char_text[prediction_idx]}"                #######May occur misplacement，Exception in line 112 in seqattack/utils/attack.py.
            ...
            ####Suitable for models: "bert" and "albert"，(lower cases in the tokenizer)####
            # current_out_token = current_out_token.lower()
            ########################################
    ...
    # print("torch.tensor(final_labels): ",torch.tensor(final_labels))
    # print(" torch.tensor(final_label_confidence: ", torch.tensor(final_label_confidence))
...
def tensor_mask(tensor):
    ...
    """
    # return torch.tensor([                                   ###Original code, only suitable for "O" in the 0th position, or skip this sample.
    #     1 if x > 0 else 0 for x in tensor
    # ])


    return torch.tensor([1] * len(tensor))                                           ###Update Eq.(1), the Denominator represents the number of labels "N".
